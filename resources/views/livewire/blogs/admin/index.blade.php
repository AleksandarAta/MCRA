    ...
    <table class="min-w-full divide-y divide-gray-200 text-sm text-left ">
        <thead class="bg-gray-100 text-gray-700 uppercase text-xs dark:text-white dark:bg-gray-800 text-center">
            <tr>
                ...
                <th class="px-4 py-2"><button wire:click = "orderBy('publish')"> {{ __('Published') }}</button></th>
        ...
        <tbody class="divide-y divide-gray-200">
            @foreach ($blogs as $blog)
                <tr class="hover:bg-gray-50 dark:hover:bg-gray-900">
                    <td class="px-3 py-2 font-medium text-gray-800 dark:text-white dark:text-bold">{{ Str::words($blog->title, 5, '...')  }}</td>
                    <td class="px-3 py-2 dark:text-white">{{ $blog->slug }}</td>
                    <td class="px-3 py-2 dark:text-white">{!! Str::words($blog->body, 5, '...') !!}</td>
                    <td class="px-3 py-2">
                        <label class="relative inline-flex items-center cursor-pointer"> 
                            <input type="checkbox" {{ $blog->publish ? 'checked' : '' }} class="sr-only peer" wire:click = "togglePublish({{ $blog->id }})">
