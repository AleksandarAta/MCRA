...
<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between" >
            ...
            <div class="flex justify-between {{ auth()->user()->hasRole('admin') ? 'w-5/12' : 'w-2/6' }}">
                @can('admin')                    
                <button
                x-on:click = "isOpen = !isOpen" class="rounded p-1 bg-orange-400  hover:text-black hover:bg-white hover:border hover:border-orange-400  dark:bg-orange-600 dark:text-white dark:hover:text-black dark:hover:bg-white dark:hover:border-orange-400 text-sm" x-text="isOpen ? '{{ __('Show as User') }}' : '{{ __('Show as Admin') }}'"></button>
                @endcan
                @can('editor')
                <button
    ...
    <div class="py-12" >
        <div class="w-2/3 mx-auto sm:px-6 lg:px-8">
            <div id="userInterface" class="bg-white dark:bg-gray-800 overflow-hidden shadow-xl sm:rounded-lg" x-show = "!isOpen" x-transition>
